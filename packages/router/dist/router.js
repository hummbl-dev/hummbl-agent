...
const promptPathForRunner = (runner) => `_state/runs/<date>/prompts/${runner}-prompt.md`;
const buildStepsForSkill = (skill, runner) => {
    const steps = [
        {
            id: "log-1",
            kind: "log",
...
const routeLlm = (input, explain) => {
    ...
    const runner = input.capabilities && input.capabilities.length > 0
...
export const route = (input) => {
    ...
    const compatible = input.skills.filter((skill) => skill.runnerCompatibility.some((runner) => input.availableRunners.includes(runner)));
    ...
    const runner = input.capabilities && input.capabilities.length > 0
        ? pickRunnerWithCapabilities(best.skill, input.availableRunners, input.capabilities)
    ...
    explain.alternatives = buildAlternatives(scored, best.skill, input.availableRunners);
    if (!policy.ok) {
        return { ok: false, error: "POLICY_DENY", explain };
    }
...
const buildAlternatives = (scored, selected, availableRunners) => {
    ...
    return scored
        ...
        .map((entry) => {
        const runner = pickRunner(entry.skill, availableRunners) ||
            entry.skill.runnerCompatibility[0];
        const reason = entry.score < selectedScore ? "lower score" : "tie-breaker";
        return {
